...
func newProduct(db *gorm.DB, opts ...gen.DOOption) product {
	...
	_product.SubCategoryID = field.NewString(tableName, "sub_category_id")
	_product.ProductName = field.NewString(tableName, "product_name")
	_product.Price = field.NewInt32(tableName, "price")
	_product.TaxStatus = field.NewInt32(tableName, "tax_status")
	_product.SubCategory = productBelongsToSubCategory{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("SubCategory", "model.SubCategory"),
		Category: struct {
			field.RelationField
		}{
			RelationField: field.NewRelation("SubCategory.Category", "model.Category"),
		},
	}
...
type product struct {
	...
	SubCategory   productBelongsToSubCategory
...
func (p *product) updateTableName(table string) *product {
	...
	p.SubCategoryID = field.NewString(table, "sub_category_id")
	p.ProductName = field.NewString(table, "product_name")
	p.Price = field.NewInt32(table, "price")
	p.TaxStatus = field.NewInt32(table, "tax_status")
...
func (p *product) fillFieldMap() {
	...
	p.fieldMap["sub_category_id"] = p.SubCategoryID
	p.fieldMap["product_name"] = p.ProductName
	p.fieldMap["price"] = p.Price
	p.fieldMap["tax_status"] = p.TaxStatus
...
type productBelongsToSubCategory struct {
	db *gorm.DB
...
func (a productBelongsToSubCategory) Where(conds ...field.Expr) *productBelongsToSubCategory {
	if len(conds) == 0 {
		return &a
	}
...
func (a productBelongsToSubCategory) WithContext(ctx context.Context) *productBelongsToSubCategory {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a productBelongsToSubCategory) Session(session *gorm.Session) *productBelongsToSubCategory {
	a.db = a.db.Session(session)
	return &a
}

func (a productBelongsToSubCategory) Model(m *model.Product) *productBelongsToSubCategoryTx {
	return &productBelongsToSubCategoryTx{a.db.Model(m).Association(a.Name())}
}

type productBelongsToSubCategoryTx struct{ tx *gorm.Association }

func (a productBelongsToSubCategoryTx) Find() (result *model.SubCategory, err error) {
	return result, a.tx.Find(&result)
}

func (a productBelongsToSubCategoryTx) Append(values ...*model.SubCategory) (err error) {
...
func (a productBelongsToSubCategoryTx) Replace(values ...*model.SubCategory) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a productBelongsToSubCategoryTx) Delete(values ...*model.SubCategory) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a productBelongsToSubCategoryTx) Clear() error {
	return a.tx.Clear()
}

func (a productBelongsToSubCategoryTx) Count() int64 {
	return a.tx.Count()
}
