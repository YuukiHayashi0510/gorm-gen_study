...
func newCategory(db *gorm.DB, opts ...gen.DOOption) category {
	...
	_category.SubCategory = categoryHasManySubCategory{
...
type category struct {
	...
	SubCategory  categoryHasManySubCategory
...
type categoryHasManySubCategory struct {
	db *gorm.DB
...
func (a categoryHasManySubCategory) Where(conds ...field.Expr) *categoryHasManySubCategory {
	if len(conds) == 0 {
		return &a
	}
...
func (a categoryHasManySubCategory) WithContext(ctx context.Context) *categoryHasManySubCategory {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a categoryHasManySubCategory) Session(session *gorm.Session) *categoryHasManySubCategory {
	a.db = a.db.Session(session)
	return &a
}

func (a categoryHasManySubCategory) Model(m *model.Category) *categoryHasManySubCategoryTx {
	return &categoryHasManySubCategoryTx{a.db.Model(m).Association(a.Name())}
}

type categoryHasManySubCategoryTx struct{ tx *gorm.Association }

func (a categoryHasManySubCategoryTx) Find() (result []*model.SubCategory, err error) {
	return result, a.tx.Find(&result)
}

func (a categoryHasManySubCategoryTx) Append(values ...*model.SubCategory) (err error) {
...
func (a categoryHasManySubCategoryTx) Replace(values ...*model.SubCategory) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a categoryHasManySubCategoryTx) Delete(values ...*model.SubCategory) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a categoryHasManySubCategoryTx) Clear() error {
	return a.tx.Clear()
}

func (a categoryHasManySubCategoryTx) Count() int64 {
	return a.tx.Count()
}
